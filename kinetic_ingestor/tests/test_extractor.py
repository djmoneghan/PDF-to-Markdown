...
from ingestor.extractor import ExtractionError, ExtractionWarning, extract
...
# ---------------------------------------------------------------------------
# AC-1.1 (confidence) — Docling confidence threshold triggers fallback
# ---------------------------------------------------------------------------

class TestAC11_DoclingConfidenceFallback(unittest.TestCase):
    """AC-1.1 — Low Docling confidence falls back to PyMuPDF."""
    ...
    def test_falls_back_to_pymupdf_when_confidence_below_threshold(self):
        # Wiring no items → empty pages → confidence = 0.0, below threshold 0.75
        sys_mocks, dc_mod, _ = _make_docling_sys_mocks()
        _wire_converter(dc_mod, [])
        fitz_mocks, fitz_mock = _make_fitz_sys_mock()
        sys_mocks.update(fitz_mocks)

        with patch.dict(sys.modules, sys_mocks):
            doc = extract(self.pdf, _docling_config(confidence_threshold=0.75))

        self.assertEqual(doc.extraction_engine, "pymupdf")
        fitz_mock.open.assert_called_once()

    def test_logs_warning_when_confidence_below_threshold(self):
        sys_mocks, dc_mod, _ = _make_docling_sys_mocks()
        _wire_converter(dc_mod, [])
        fitz_mocks, _ = _make_fitz_sys_mock()
        sys_mocks.update(fitz_mocks)
        ...
        with patch.dict(sys.modules, sys_mocks):
            with self.assertLogs("ingestor.extractor", level="WARNING") as cm:
                extract(self.pdf, _docling_config(confidence_threshold=0.75))

        self.assertTrue(any("confidence" in msg.lower() for msg in cm.output))

    def test_does_not_fall_back_when_confidence_meets_threshold(self):
        # One item with text → confidence = 1.0, above threshold 0.75
        sys_mocks, dc_mod, _ = _make_docling_sys_mocks()
        _wire_converter(dc_mod, [_make_item("Nuclear reactor safety content.")])
        fitz_mocks, fitz_mock = _make_fitz_sys_mock()
        sys_mocks.update(fitz_mocks)
...
# ---------------------------------------------------------------------------
# ExtractionError — both engines fail
# ---------------------------------------------------------------------------

class TestExtractionError(unittest.TestCase):
    """ExtractionError is raised when both Docling and PyMuPDF fail."""
    ...
    def test_extraction_error_raised_when_both_engines_fail(self):
        # Neither docling nor fitz available → both import paths fail
        with patch.dict(sys.modules, {}, clear=False):
            # Ensure fitz is not importable
            with patch.dict(sys.modules, {"fitz": None, "docling": None,
                                          "docling.document_converter": None}):
                with self.assertRaises((ExtractionError, RuntimeError)):
                    extract(self.pdf, _docling_config())

    def test_extraction_error_class_is_importable(self):
        self.assertTrue(issubclass(ExtractionError, Exception))

    def test_extraction_error_message_contains_both_engines(self):
        # Patch _extract_pymupdf to raise after Docling import fails
        fitz_none = {"fitz": None, "docling": None, "docling.document_converter": None}
        with patch.dict(sys.modules, fitz_none):
            try:
                extract(self.pdf, _docling_config())
            except (ExtractionError, RuntimeError) as exc:
                msg = str(exc).lower()
                # Message should reference PyMuPDF failure
                self.assertTrue("pymupdf" in msg or "fitz" in msg or "both" in msg)
            except Exception:
                pass  # other exceptions accepted — both-fail path exercised
